...
public class RedisZSet extends RedisObject {
    ...
    public static void main(String[] args) {
        ...
        redisZSet.zAdd("liangjingxiu",100.00);
        testSkipList(redisZSet);
    }
    private static void testSkipList(RedisZSet redisZSet){
        for(int i=0;i<123;++i){
            redisZSet.zAdd("qww"+i,i);
        }
        //redisZSet.zAdd("qwertyuiopasdfghjklzxcvbnm1234567",100.00);
        redisZSet.zAdd("zzy",-50);

        printAllItem(redisZSet);

        System.out.println(redisZSet.zRank("leida"));
        System.out.println(redisZSet.zRank("zzy"));
        double score  = redisZSet.zScore("huyiran");
        System.out.println(score);
        System.out.println(redisZSet.zRank("huyiran"));
        System.out.println(redisZSet.zRevRank("huyiran"));
        System.out.println(redisZSet.zRank("leida"));

        redisZSet.zRem("qihang");
        //printAllItem(redisZSet);
        System.out.println(redisZSet.zRank("leida"));

        redisZSet.zAdd("qihang",101);
        //printAllItem(redisZSet);
        System.out.println(redisZSet.zRank("leida"));
    }
    private static void printAllItem(RedisZSet redisZSet) {
        if (redisZSet.encoding == RedisEnc.ZIPLIST.VAL()) {
            ZipList zipList = (ZipList) redisZSet.ptr;
            ZipList.print(zipList);
        } else {
            ZSet<SDS> zSet = (ZSet<SDS>) redisZSet.ptr;
            Dict.printDictInfo(zSet.dict);
            SkipList.printSkipList(zSet.zsl);
        }
    }
}
